...
class TokenValidationViewTests(TestCase):
    ...
    def test_missing_session_jti(self):
        ...
        response = self.client.post("/token-validation/", data={"access": "a", "refresh": "r"}, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_blacklisted_tokens(self, mock_redis):
        ...
        mock_conn = MagicMock()
        mock_conn.get.side_effect = lambda key: "something" if "black_list" in key else None
    ...
    def test_valid_access_token(self, mock_redis):
        ...
        mock_conn = MagicMock()
        mock_conn.get.side_effect = lambda key: "hash" if "access_token" in key else None
        mock_redis.return_value = mock_conn
    ...
    def test_expired_access_valid_refresh_generates_new_tokens(self, mock_build, mock_redis):
        ...
        mock_conn.set.assert_any_call("black_list:valid-refresh", "valid-refresh", ex=3600)
