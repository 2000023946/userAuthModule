...
class RegistrationTests(APITransactionTestCase):
    ...
    def test_register_creates_user_step_by_step(self):
        ...
        response = self.client.post(
            url, {"email": "newuser@example.com"}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        response = self.client.post(
            url, {"password_repeat": "ValidPass123!"}, format="json"
        )
    ...
    def test_register_fails_on_password_mismatch(self):
        ...
        response = self.client.post(
            url, {"password_repeat": "WrongPass123!"}, format="json"
        )
    ...
    def test_register_fails_if_username_similar_to_email(self):
        ...
        self.assertFalse(
            CustomUser.objects.filter(username="user@example.com").exists()
        )
