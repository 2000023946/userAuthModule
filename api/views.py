...
from .builder import UserBuilder, PasswordResetBuilder, LoginBuilder
...
class AuthViews(APIView):
    ...
    def post(self, request):
        ...
        result = service.execute(request, self.builder(request=request), self.factoryState().build())
        print('result', result)
    ...
    def post(self, request):
        try:
            ...
            if conn.ttl(f'refresh_token:{refresh_token}') > 0:
                conn.set(f'black_list:{refresh_token}', refresh_token, ex=conn.ttl(f'refresh_token:{refresh_token}'))

            if conn.ttl(f'access_token:{access_token}') > 0:
                conn.set(f'black_list:{access_token}', access_token, ex=conn.ttl(f'access_token:{access_token}'))
...
class ThirdPartyView(AuthViews):
    def post(self, request):
        ...
        try:
            ...
            # Convert any error from get_user_info into a 400 JSON response
            return Response({'errors': str(exc)}, status=status.HTTP_400_BAD_REQUEST)

        # At this point user_info is safe to pass to AuthViews.post
...
# -------------------------------
# 4️⃣ Access and Refresh endpoints
# -------------------------------
...
class TokenValidationView(APIView):
    ...
    def post(self, request):
        ...
        if not access_token_str or not refresh_token_str:
            return Response({"detail": "Tokens required"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        #check if the access_token_str and the refresh_token_str not blacklisted
        if conn.get(f'black_list:{access_token_str}') or conn.get(f'black_list:{refresh_token_str}'):
            return Response({"detail": "Expired Tokens"}, status=status.HTTP_400_BAD_REQUEST)
        
        #check if the access is valid
        if conn.get(f'access_token:{access_token_str}'):
            return Response({"detail": "Success access token valid"}, status=status.HTTP_200_OK)
        
        #check if the refresh is expired
        if not conn.get(f'refresh_token:{refresh_token_str}'):
            return Response({"detail": "Invalid Refresh Token"}, status=status.HTTP_400_BAD_REQUEST)
        
        #blacklist current refresh since the access token is already expired, but the refresh is still not expired, 
        # so we are giving new refresh and access

        #blacklist the refresh token
        conn.set(f'black_list:{refresh_token_str}', refresh_token_str, ex=conn.ttl(f'refresh_token:{refresh_token_str}'))
